...
contract("Dex", accounts => {
    it("should be only possible to BUY if user has enough ETH", async => {
        ...
        await truffleAssert.reverts(
            dex.createLimitOrder(0, "TST", 10, 20, {from: accounts[0]})
        );

        await dex.depositWei({from: accounts[0], value: 1000});
    ...
    it("should be only possible to SELL if user has enough TOKEN", async => {
        ...
        await truffleAssert.reverts(
            dex.createLimitOrder(1, "TST", 10, 20, {from: accounts[0]})
        );
    ...
    it("should be that BUY orders are ORDERED (lol) from highest to lowest", async => {
        ...
        dex.createLimitOrder(0, "TST", 10, 20, {from: accounts[0]});
        dex.createLimitOrder(0, "TST", 3, 40, {from: accounts[0]});
        dex.createLimitOrder(0, "TST", 5, 15, {from: accounts[0]});
    ...
    it("should be that SELL orders are ORDERED (lol) from lowest to highest", async => {
        ...
        dex.createLimitOrder(1, "TST", 5, 32, {from: accounts[0]});
        dex.createLimitOrder(1, "TST", 3, 58, {from: accounts[0]});
        dex.createLimitOrder(1, "TST", 5, 22, {from: accounts[0]});
