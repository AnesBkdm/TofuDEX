...
/**
 * @author Nes B.
 * @dev Decentralized Exchange implementation for TofuDEX.
...
contract Dex is Wallet {
    ...
    /**
     * @dev Basic trading order struct used in the entire project.
     */
    struct Order {
        uint id;
        address trader;
    ...
    /**
     * @dev Creates and processes MARKET ORDER.
     * 
     * The function looks at the side of the submitted order. If it is a SELL, 
     * it checks out immediately the token balance of the caller. It then calls
     * the orderbook with the opposite SIDE, fills the submitted order with the
     * available limit orders, then cleans the orderbook.
     */
    ...
    /**
     * @dev Creates and stores LIMIT ORDER.
     *
     * Checks the balance of caller depending on side, pushes the order in the
     * orderbook, then sorts the latter with bubble sort algorithm.
     */
